...
#+BEGIN_SRC emacs-lisp
  (require-packages 'command-log-mode)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (require-packages 'aggressive-indent)
...
#+RESULTS:
| lambda | nil | (set (make-local-variable (quote indent-tabs-mode)) (quote nil)) | (set (make-local-variable (quote js-indent-level)) 2) | (set (make-local-variable (quote tab-width)) 2) | (add-untabify-before-save) |
...
#+RESULTS:
| (lambda nil (tern-mode t)) | skewer-mode | (lambda nil (slime-js-minor-mode 1)) | (lambda nil (define-key js2-mode-map i (quote js-doc-insert-function-doc)) (define-key js2-mode-map @ (quote js-doc-insert-tag))) | (lambda nil (set (make-local-variable (quote indent-tabs-mode)) (quote nil)) (set (make-local-variable (quote js-indent-level)) 2) (set (make-local-variable (quote tab-width)) 2) (add-untabify-before-save)) | er/add-js2-mode-expansions | er/add-js-mode-expansions |
...
#+BEGIN_SRC emacs-lisp
  (require-packages 'company-tern)
  (eval-after-load 'company
      '(add-to-list 'company-backends 'company-tern))
#+END_SRC

#+RESULTS:
| company-tern | alchemist-company | company-bbdb | company-nxml | company-css | company-eclim | company-semantic | company-clang | company-xcode | company-cmake | company-capf | company-files | (company-dabbrev-code company-gtags company-etags company-keywords) | company-oddmuse | company-dabbrev |
...
Support for adding jsdoc style comments to javascript files.

I am more tempted by docco style documentation, but a lot of others
seem to prefer the js-doc style of structured comments.

#+BEGIN_SRC emacs-lisp
  (require-packages 'js-doc)
  (setq js-doc-mail-address "pti@melexis.com"
        js-doc-author (format "Peter Tillemans <%s>" js-doc-mail-address)
        js-doc-url "https://www.melexis.com/"
        js-doc-license "Apache 2")
...
#+RESULTS:
| (lambda nil (define-key js2-mode-map i (quote js-doc-insert-function-doc)) (define-key js2-mode-map @ (quote js-doc-insert-tag))) | skewer-mode | (lambda nil (slime-js-minor-mode 1)) | (lambda nil (set (make-local-variable (quote indent-tabs-mode)) (quote nil)) (set (make-local-variable (quote js-indent-level)) 2) (set (make-local-variable (quote tab-width)) 2) (add-untabify-before-save)) | er/add-js2-mode-expansions | er/add-js-mode-expansions |
...
- C-c Ret:   Send current statement to shell
- C-M-x:     Send current toplevel block to shell
- C-c C-z:   Switch to python shell
- M-x elpy-use-ipython: choose ipython shell
- M-x elpy-use-cpython: choose cpython shell
...
- M-Tab:     Accept proposal
- M-n, M-p:  Browse proposals use Ret to accept, Tab to expand common
- C-d:       Show documentation
- C-w:       Show source of proposed completion
...
#+BEGIN_SRC emacs-lisp
  ...
  (elpy-enable)
  (elpy-use-ipython)
  (setq elpy-rpc-python-command "python3") ;; use the default python3
  (elpy-rpc-restart)
#+END_SRC
...
(pyvenv-activate (expand-file-name "~/anaconda/envs/pelican"))
#+END_SRC

this would activate the pelican environment. For the root environment
use :
