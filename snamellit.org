...
#+BEGIN_SRC emacs-lisp
  ...
  (defun pti-path-from-shell ()
    (pti-chomp
     (shell-command-to-string "$SHELL -l -c 'echo -n $PATH'")))

  (defun pti-set-exec-path-from-shell-PATH ()
    "Sets the exec-path to the same value used by the user shell"
    (let* ((path-from-shell (pti-path-from-shell))
           (path-list (mapcar 'pti-chomp
                              (split-string path-from-shell path-separator))))
      (message (concat "Path from shell: <" path-from-shell ">"))
      (setenv "PATH" path-from-shell)
      (setq exec-path path-list)))

  ;; when running in a GUI reset path from an initialized shell
  (when window-system (pti-set-exec-path-from-shell-PATH))

#+END_SRC

#+RESULTS:
| /usr/local/heroku/bin | /usr/local/bin | /usr/local/sbin | /Users/pti/anaconda3/envs/pelican/bin | /usr/bin | /bin | /usr/sbin | /sbin | /opt/X11/bin | /Library/TeX/texbin | /Applications/Wireshark.app/Contents/MacOS | /Users/pti/bin | /Users/pti/playpen/go/bin | /usr/local/opt/go/libexec/bin | /Users/pti/.cargo/bin | /Users/pti/anaconda3/bin | /Users/pti/Library/Haskell/bin\n | /Users/pti/Library/Haskell/bin\n |
...
** Utility functions to login to webservers


Note this implementation only works with the *url.el* backend and not
with the *curl* backend

#+BEGIN_SRC emacs-lisp
  ;;; advice only works with the url-backend
  (setq request-backend 'url-backend)

  (defun pti-make-digest (host port)
    "Create a basic authentication digest if credentials known.

  Checks the auth-source database if there is a username/password defined
  for the host and port and calculates the base64 version needed to add
  to an *Authorization* header in a HTTP request.

  Returns nil if no credentials were found for the host/port combination.
  "
    (let ((found (nth 0 (auth-source-search :max 1
                                            ...
                                            :require '(:user :secret)))))
      (if found
          (let ((username (plist-get found :user))
                (password (plist-get found :secret)))
            (concat "Basic "
                    (base64-encode-string
                     (concat username ":" (if (functionp password)
                                              (funcall password)
                                            password)))))
        nil)))


  (defun pti-make-url-request-extra-headers (host port)
    "Adds a authorization header if credentials are known

  Generates a Basic Authentication header if credentials for the host and
  port combination are known and returns an alist containing the current
  values of the dynamically bound url-request-extra-headers with the
  authorization header added.

  The return value can be directly used te rebind the
  url-request-extra-headers variable.
  "
    (let ((digest (pti-make-digest host port)))
      (if digest
          (cons `("Authorization" . ,digest) url-request-extra-headers)
        url-request-extra-headers)))

  (defun pti-add-authorization-header (orig-fun url &rest args)
    "Calls the original function after adding the auth headers

  Checks in the auth-source database if credentials are present for the
  host/scheme combination in the url and adds an Authorization header
  for Basic Authentication to the url-request-extra-headers before
  calling the original function.

  Add this function as advice to url-retrieve :

    (advice-add 'url-retrieve :around #'pti-add-authorization-header )

  To remove this advice use :
    (advice-remove 'url-retrieve #'pti-add-authorization-header)

  "
    (let* ((url-obj (url-generic-parse-url url))
           (url-request-extra-headers (pti-make-url-request-extra-headers (url-host url-obj) (url-type url-obj))))
  ...
  ;;; (advice-remove 'url-retrieve #'pti-add-authorization)

#+END_SRC

#+RESULTS:
: pti-add-authorization-header
...
Switching windows with C-x o quickly becomes an exercise in
frustration. So some new keybindings are in order:
...
These keybindings allow quickly to jump between 2 windows
...
This installs a hydra to enable navigation and structure editing of
indented code like yaml, python, ...
...
#+BEGIN_SRC emacs-lisp
  (use-package smart-parens-config
    :ensure smartparens
    :config (progn
...
Enable smartparens and rainbow delimiters in the repl. We also let the
...
There is support for flow in the vendor folder but it defines global
keybindings which conflict all over the place.
...
Smartparens allows you to enter lisp code where it is actually impossible
...
#+RESULTS:
| lambda | nil | (smartparens-mode 1) | (show-paren-mode 1) | (add-untabify-before-save) |
| lambda | nil | (paredit-mode 1)     | (show-paren-mode 1) | (add-untabify-before-save) |
...
** DONE Rust
   - State "DONE"       from "TODO"       [2017-01-13 Fri 10:40]
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package racer
    ...
    (setq racer-rust-src-path "/Users/pti/.multirust/toolchains/stable-x86_64-apple-darwin/lib/rustlib/src/rust/src")
    :init
  ...
  (use-package cargo
    ...
    (add-hook 'cargo-process-mode-hook (lambda () (visual-line-mode 1))))
...
I found out about *Intero* mode and how would enable me to cut whole
swaths of config code from my Emacs config and I was sold.

Intero uses a special process per project to do the heavy lifting of
the language interpretation to give feedback while developping,
similar to what ghc-mod does. I was bitten a few days back when
switching between projects that this system got seriously confused and
started reading libraries from the wrong place for the compiler of the
project.

Intero does have a large startup cost the first time it is used in a
project as it will adds it's own dependency to the sandbox. In view of
recent experiences I am happy to pay this cost.
...
*** Useful shortcuts
 ...
 - C-c C-l: Load or reload file in repl and open it
 - C-c C-z: toggle between repl and source
 - C-c C-n C-c: cabal build project
 - C-c C-n c: interactive cabal build command
 - C-c C-n C-t: give type info about thing under cursor
 - C-c C-n C-i: give info about thing under cursor

note: sometimes it is needed to check the results of the info
shortcuts in the *Messages* buffer.
...
#+RESULTS:
| m | TODO from Mail | entry | (file+headline ~/org/gtd.org Inbox)          | * TODO %?, Link: %a |
...
as it interrupts the flow at (seemingly) random times.
...
Bootstrap the xml schemas with the seeded schemas collected in
*~/.emacs.d/schema* folder.

Currently this contains *B2MML* and  *DocBook*.

#+BEGIN_SRC emacs-lisp
  (use-package nxml
    :init (push "~/.emacs.d/schema/schema.xml"
...
(use-package es-mode
  :mode ("\\.es" . es-mode)
  :config (add-hook 'es-result-mode-hook 'hs-minor-mode))
#+END_SRC
...
Elasticsearch mode adds better support for elasticsearch queries in a
buffer.
#+BEGIN_SRC emacs-lisp

  (setq pti-google-client-id "36868217908-4qetjh9iaoi404pi2e9ffjimteaun67h.apps.googleusercontent.com")
  (setq pti-google-client-secret "lW-cKXGfGV91BdaL64LclxTg")
...
- C-c C-c : execute query and show results in buffer
